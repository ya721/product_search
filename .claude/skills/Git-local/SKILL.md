---
name: git-local
description: ローカルGit運用を厳格に管理します。バックアップブランチの作成、GitFlow準拠のブランチ戦略、Conventional Commits形式での原子的コミット、コンフリクト解決、ローカルレビューを徹底。リモート操作・PR・CIを使わず、全てローカルで完結させる安全な運用を実現します。
---
